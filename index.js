const { Observable: { create }, interval, from, Observable, operators: { reduce, bufferTime, bufferCount } } = rxjs;
// 1. Имплементировать функцилнал range используя функцию create.
const analogRange = (startingIndex, finalIndex) => {
...
const analogFilter = (callback) => (source) => {
    return source.pipe(reduce((acc, val) => {
        if (callback(val))
            acc.push(val);
        return acc;
    }, []));
...
const subscriber4 = publisher4.subscribe((val) => {
    console.log(val);
});
